...
def send_videos_to_api(videos_data):
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            print(error_msg)
            _log_error_for_all_videos(
                videos_data, f"{error_msg}. Response: {response.text}")
            return
...
def _handle_api_response(response, videos_data):
    ...
    try:
        ...
        for error in errors:
            ...
            if video_name:
                matched_video = next(
                    (v for v in videos_data if v['name'] == video_name), None)
        ...
        _log_error_for_all_videos(
            videos_data,
            f"Failed to parse API response: {response.text}")

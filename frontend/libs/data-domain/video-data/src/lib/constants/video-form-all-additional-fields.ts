import { VideoFormModelFieldsEnum } from '@frontend/video-data';

export const VideoFormAllAdditionalFields: VideoFormModelFieldsEnum[] = [
  VideoFormModelFieldsEnum.GAME_SYSTEM,
  VideoFormModelFieldsEnum.WEAPON_TYPE,
  VideoFormModelFieldsEnum.TOPIC,
  VideoFormModelFieldsEnum.GUESTS,
  VideoFormModelFieldsEnum.TEAM_ONE_ID, 
  VideoFormModelFieldsEnum.TEAM_TWO_ID, 
  VideoFormModelFieldsEnum.TEAM_ONE_CITY, 
  VideoFormModelFieldsEnum.TEAM_TWO_CITY, 
  VideoFormModelFieldsEnum.TEAM_ONE_MIX, 
  VideoFormModelFieldsEnum.TEAM_TWO_MIX,
  VideoFormModelFieldsEnum.TOURNAMENT_ID, 
  VideoFormModelFieldsEnum.TOURNAMENT_CITY, 
  VideoFormModelFieldsEnum.TOURNAMENT_START_DATE, 
  VideoFormModelFieldsEnum.TOURNAMENT_END_DATE,
];